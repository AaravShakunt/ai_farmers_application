    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50/30 via-emerald-50/20 to-green-100/40">
      <div className="mx-auto max-w-screen-md px-4 pb-24 pt-4 md:px-6">
        ...
        <div className="mb-6">
          ...
          <div className="bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl shadow-lg p-6 mb-6 text-white">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h1 className="text-2xl font-bold">{workflow.plotName}</h1>
                <p className="text-green-100 opacity-90">{workflow.crop} Farm Workflow</p>
              </div>
              <div className="text-right">
                <div className="text-3xl font-bold">{completedTasks}/{totalTasks}</div>
                <div className="text-xs text-green-100 opacity-80">Tasks Completed</div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center text-sm">
              <span className="text-green-100 opacity-80">Progress: {Math.round(progressPercentage)}%</span>
              <span className="text-green-100 opacity-80">Updated: {new Date(workflow.lastUpdated).toLocaleDateString()}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-800">Farm Tasks</h2>
          ...
          {workflow.tasks.map((task, index) => (
            <div key={task.id} className={`bg-white rounded-xl border shadow-sm overflow-hidden transition-all duration-200 hover:shadow-md ${task.completed ? 'bg-gray-50 border-gray-200' : 'border-gray-200 hover:border-green-300'}`}>
              <div className="p-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <h3 className={`font-semibold text-base ${task.completed ? 'line-through text-gray-500' : 'text-gray-800'}`}>
                          {task.title}
                        </h3>
                      ...
                      <div className="flex items-center mt-2">
                        <span className={`text-xs font-medium ${task.completed ? 'text-gray-400' : 'text-gray-500'}`}>
                          Task #{index + 1}
                        </span>
                        {task.completed && (
              ...
              {!task.completed && (
                <div className="h-1 bg-gradient-to-r from-blue-200 to-green-200"></div>
              )}
              {task.completed && (
          ...
          {workflow.tasks.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Tasks will appear here when generated from your farm activities.</p>
            </div>
          )}
        </div>
